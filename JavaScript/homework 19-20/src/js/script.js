...
var ownerArr = JSON.stringify(data);

(function() {
  var obj = JSON.parse(ownerArr);


  var skills = _.sortedUniq(_.sortBy(_.map(_.flatten(_.map(obj, 'skills')), _.toLower)));
  console.log('1. Alphabetically ordered array of people skills without repetitions: ', skills);


  var  people = _.map(obj, function(data) {
    return { "name": data.name, "friends": data.friends.length }
  });

  var peopleNames  = _.map(_.sortBy(people, "friends"), "name");
  console.log('2. An array of people names ordered by friends quantity :', peopleNames);


  
  var friends = _.sortedUniq(_.map(_.sortBy(_.flatten(_.map(obj, "friends")), "name"), "name"));
  console.log('3. An array of people friends without repetitions :', friends);
